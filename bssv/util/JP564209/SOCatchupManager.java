package be.e1.bssv.util.JP564209;

import be.e1.bssv.util.J564209.SOCatchupMgr;
import be.e1.bssv.util.J564209.valueobject.InternalSOCatchupDS;
import be.e1.bssv.util.JP564209.valueobject.SOCatchUpReply;
import be.e1.bssv.util.JP564209.valueobject.SOCatchUpRequest;

import oracle.e1.bssvfoundation.base.IContext;
import oracle.e1.bssvfoundation.base.PublishedBusinessService;
import oracle.e1.bssvfoundation.connection.IConnection;
import oracle.e1.bssvfoundation.exception.BusinessServiceException;
import oracle.e1.bssvfoundation.util.E1MessageList;
import oracle.e1.bssvfoundation.util.MathNumeric;

/**
 * TODO: Java Doc for PublishedBusinessService
 */
public class SOCatchupManager extends PublishedBusinessService {
    /**
     * TODO: PublishedBusinessService Public Constructor
     */
    public SOCatchupManager() {
    }

    /**
     * Published method for generateSOCatchupData TODO: Document method
     * Acts as wrapper method, passing null context and null connection,
     * will call protected generateSOCatchupData
     * @param vo the value object representing input data for TODO:complete javadoc
     * @return confirmVO the response data from the business process TODO:complete javadoc
     * @throws BusinessServiceException
     */
    public SOCatchUpReply generateSOCatchupData(SOCatchUpRequest vo) throws BusinessServiceException {
        return generateSOCatchupData(null,null,vo);
    }

    /**
     * Protected method for SOCatchupManager PublishedBusinessService. generateSOCatchupData will call make calls to BusinessService classes
     * for completing business process.
     * TODO: Comment parameters
     * @param vo the value object representing input data for TODO:complete javadoc
     * @param context conditionally provides the connection for the database operation and logging information
     * @param connection can either be an explicit connection or null. If null the default connection is used.
     * @return response value object is the data returned from the business process TODO:complete javadoc
     * @throws BusinessServiceException
     */
    protected SOCatchUpReply generateSOCatchupData(IContext context, IConnection connection, SOCatchUpRequest vo) throws BusinessServiceException {
        //perform all work within try block, finally will clean up any connections.
        try {
            //Call start published method, passing context of null
            //will return context object so BSFN or DB operation can be called later.
            //Context will be used to indicate default transaction boundary, as well as access
            //to formatting and logging operations.
            context = startPublishedMethod(context, "generateSOCatchupData", vo);

            //Create new PublishedBusinessService messages object for holding errors and warnings that occur during processing.
            E1MessageList messages = new E1MessageList();

            InternalSOCatchupDS internalDS = new InternalSOCatchupDS();
            internalDS.setMnOrderNumber(new MathNumeric(vo.getOrderNumber()));
            internalDS.setMnSoldToNumber(new MathNumeric(vo.getSoldToAddressNumber()));
            internalDS.setSzOrderCompany(vo.getOrderCompany());
            internalDS.setSzOrderType(vo.getOrderType());

            messages.addMessages(SOCatchupMgr.createSOCatchupData(context, connection, internalDS));

            //PublishedBusinessService will send either warnings in the Confirm Value Object or throw a BusinessServiceException.
            //If messages contains errors, throw the exception

            if (messages.hasErrors()) {
                //Get the string representation of all the messages.
                String error = messages.getMessagesAsString();
                //Throw new BusinessServiceException
                throw new BusinessServiceException(error, context);
            }

            //Exception was not thrown, so create the confirm VO from internal VO
            SOCatchUpReply confirmVO = new SOCatchUpReply(internalDS);
            confirmVO.setE1MessageList(messages);
            finishPublishedMethod(context, "generateSOCatchupData");
            //return outVO, filled with return values and messages
            return confirmVO;
        } finally {
            //Call close to clean up all remaining connections and resources.
            close(context, "generateSOCatchupData");
        }
    }
}
